...
流程: 自动循环 N 次 -> 设置自定义起手位姿 -> 停留 1 秒等待静止 -> 开启录制 
      ...
      -> 以文件夹形式保存图片序列、npz 状态和自然语言指令 (含 5% 概率模拟失误与纠错)。
...
# 模拟人类示范失误：以该概率故意抓偏一次再张开重抓(纠错示范)
RETRY_PROB = 0.05

GRIP_OPEN = rspec.GRIP_OPEN      # (-0.02, +0.02)  负值 = 张开
GRIP_CLOSE = rspec.GRIP_CLOSE    # (+0.02, -0.02)  正值 = 闭合
GRIP_INIT = rspec.GRIP_INIT      # 初始/默认状态：闭合(仅在接近并抓取螺丝刀时才张开)
...
# VLA 数据集录制配置（部署端按同样的值决定每个动作跑多少个 mj_step）
CONTROL_HZ = rspec.CONTROL_HZ
SIM_TIMESTEP = rspec.SIM_TIMESTEP
STEPS_PER_RECORD = rspec.STEPS_PER_RECORD
...
class VLARecorder:
    ...
    def __init__(self, model, data):
        ...
        # 🔗 屏蔽选项与渲染器都走共享入口，部署端用同一份，保证画面逐像素同构
        self.vopt = rspec.make_scene_option()
        self.renderer_global = rspec.make_renderer(model)
        self.renderer_wrist = rspec.make_renderer(model)
    ...
    def record_step(self, ctrl_target):
        if self.step_count % STEPS_PER_RECORD == 0:
            self.renderer_global.update_scene(self.data, camera=rspec.CAM_FIXED, scene_option=self.vopt)
            self.obs_global.append(self.renderer_global.render().copy())
            
            self.renderer_wrist.update_scene(self.data, camera=rspec.CAM_WRIST, scene_option=self.vopt)
...
def pick_and_place(model, data, viewer, arm_act_ids, j8_id, j9_id, ik, plan,
    ...
    """
    真实执行流程。包含 5% 概率触发抓取失误与纠错逻辑（模拟人类示范时的修正）。
    ...
    intentional_miss = (np.random.rand() < RETRY_PROB) if not is_dry_run else False
    has_retried = False
    ...
    while True:
        ...
        if intentional_miss and not has_retried:
            print("    ⚠️ 触发 5% 概率抓取偏移 (模拟纠错示范)，准备张开夹爪重试...")
...
def collect_dataset(args):
    ...
    #    部署端调用的是同一个函数，所以不可能再出现"一端做了镜像另一端没做"。
    # ---------------------------------------------------------
    model, data, ridx = rspec.load_scene()
    mujoco.mj_resetData(model, data)

    arm_act_ids = ridx.arm_act_ids
    j8_id = ridx.j8_id
    j9_id = ridx.j9_id
    ik = ArmIK(model)
